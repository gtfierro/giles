...
class ZC(driver.SmapDriver):
    def setup(self, opts):
        ...
        for floor in range(1, num_floors):
            query = "Metadata/Building = 'UC Berkeley' and Metadata/Floor = '{floor}' and Metadata/Sensor/Measure = 'Temperature'".format(floor=floor)
        ...
        for floor in range(1, num_floors):
            ...
            for zone in range(1, num_zones_per_floor):
                query = "Metadata/Building = 'UC Berkeley' and Metadata/Zone = '{zone}' and Metadata/Sensor/Measure = 'Occupancy'".format(zone=str(floor*1000+zone*10))
                rc = RepublishClient(self.archiver_url, self.cb, restrict=query)
